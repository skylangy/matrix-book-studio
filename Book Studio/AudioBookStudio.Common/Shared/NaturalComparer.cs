...
public class NaturalComparer(StringComparison stringComparison = StringComparison.Ordinal) : IComparer<string?>, IComparer<ReadOnlyMemory<char>>
{
    public static readonly NaturalComparer Instance = new(StringComparison.Ordinal);
    ...
    public static int Compare(ReadOnlySpan<char> x, ReadOnlySpan<char> y, StringComparison stringComparison)
    {
        ...
        for (var i = 0; i < length; i++)
        {
            ...
            {
                var xOut = GetNumber(x[i..], out var xNum);
                var yOut = GetNumber(y[i..], out var yNum);
