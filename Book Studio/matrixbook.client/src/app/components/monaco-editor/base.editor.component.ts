...
import { MONACO_EDITOR_CONFIG, MonacoEditorConfig } from './models';
...
export abstract class BaseEditor implements AfterViewInit, OnDestroy {
    ...
    @Output() beforeEditorInit = new EventEmitter<any>();
    @Output() afterEditorInit = new EventEmitter<any>();
    ...
    ngAfterViewInit(): void {
        if (loadedMonaco) {
            ...
            loadPromise.then(() => {
                this.beforeEditorInit.emit(this._options);
                this.initMonaco(this._options, this.insideNg);
            });
        } else {
            loadedMonaco = true;
